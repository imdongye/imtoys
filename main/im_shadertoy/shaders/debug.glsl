...
23 SUCCESSIVE COORDINATE TRANSFORMATIONS 회전행렬
...
//
// "smoothstep" function is like step function but instead대신에 of a
...
void mainImage( out vec4 FragColor, in vec2 fragCoord )
{
	...
	if(p.x < 1./5.) { // Part I
		...
		// implementation of smoothstep 완성
		float edge0 = 0.45;
		float edge1 = 0.55;
		...
		// To achieve smoothness, implement a cubic Hermite polynomial 에르미트 다항식적용
		// 3*t^2 - 2*t^3
		ret = t1;
	}
		...
		// smootherstep,더부드러운 a suggestion제안 by Ken Perlin 펄린
		float edge0 = 0.45;
		float edge1 = 0.55;
		float t = clamp((p.y - edge0)/(edge1 - edge0), 0.0, 1.0);		
		// 6*t^5 - 15*t^4 + 10*t^3
		...
		// but computationally more involved. 더많은 계산을 필요로함
	}	
...
void mainImage( out vec4 FragColor, in vec2 fragCoord )
{
	...
	if(p.x < 1./5.) { // Part I
		...
		float m = 1.0; // amount of mix (between 0.0 and 1.0)
...
void mainImage( out vec4 FragColor, in vec2 fragCoord )
{
	...
	float radius = 0.2+abs(sin(iTime)*0.2); // increase this to see the effect better
...
// Instead대신에 use the set of (x,y) that satisfy: -d < y-f(x) < d
...
// if you change their order, you get differentults.
//
...
void mainImage( out vec4 FragColor, in vec2 fragCoord )
{
	...
	if(p.x < 1./5.) { // Part I
		...
		float speed = 2.0;
		float t = iTime*speed;
		float stopEveryAngle = PI/4.0;
        
		float stopRatio = 0.7;
        float moveLength = 0.6;
        float moveSpeed = 1.0;
        
        //이해해
		float t1 = (floor(t) + smoothstep(0.0, 1.0-stopRatio, fract(t)) )*stopEveryAngle;
        
		float t2 = moveLength*(floor(t)+smoothstep(0.0, 1.0-stopRatio, fract(t)));
